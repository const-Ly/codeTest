    ...
    methods:{
      ...
      showErrTip (data) {
        ...
        if(data.failCount > 0){// 失败条数不为0
          this._html = "成功导入"+data.successCount+"条数据，失败"+data.failCount+"条，以下为失败数据所在行号，请注意粘贴保存检查：<span class='font-danger'>"+data.failLineNumberList.join("、")+"</span>。"
        }else{
          this._html = "成功导入"+data.successCount+"条数据，失败"+data.failCount+"条。"
        }
        this.showTip = true
        setTimeout(()=>{
