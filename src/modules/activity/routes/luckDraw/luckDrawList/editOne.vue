    ...
    <el-card class="box-card">
      ...
      <el-form ref="form" label-width="120px" size="small" class="input-normal">
        ...
        <el-form-item label="组队类型：">
          <el-radio-group v-model="param.groupType" disabled>
        ...
        <el-form-item label=" " v-if="param.groupType !== 0">
          <template v-if="param.groupType === 1">
            <el-input v-model="param.forceGroupNum" v-only-num="param.forceGroupNum" :max-len="2" disabled class="w100"></el-input> 人/队
          </template>
          <template v-if="param.groupType === 2">
            <el-input v-model="param.nonForceGroupNum" v-only-num="param.nonForceGroupNum" :max-len="2" disabled class="w100"></el-input> 人/队
          </template>
        </el-form-item>
        <el-form-item label="奖品来源：">
          <el-radio-group v-model="param.source" disabled>
            <el-radio :label="0">平台奖品</el-radio>
            <el-radio :label="1">商家奖品</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="奖品数量：">
          <el-input v-model="param.rewardNum" disabled v-only-num="param.rewardNum" :max-len="8" class="w100"></el-input>
          <span class="font font-info">（强制组队必须为组队人数的整数倍，非强制组队必须大于等于组队人数）</span>
        </el-form-item>
        <el-form-item label="虚拟参与人：" v-if="param.source === 0">
          <el-input v-model="param.rebotNum" disabled v-only-num="param.rebotNum" :max-len="8" class="w100"></el-input>
...
<script type="text/javascript">
  export default {
    ...
    data () {
      let self = this
      return {
        lotteryId:self.$route.params.id,
        startTime:null,
        lotteryTime:null,
    ...
    mounted () {
      if(localStorage.getItem('onlyEdit')){
        this.handleCache()
      }else{
        this.getLotteryInfo()
      }
    ...
    methods:{
      ...
      getLotteryInfo () {
        this.$service.activity.luckDraw.editLottery({lotteryId: this.lotteryId}).then(res => {
          this.param = res
          this.param.lotteryNum = this.param.lotteryNum || ''
          this.startTime = new Date(res.startTime)
          if(res.lotteryTime !== ""){
            this.lotteryTime = new Date(res.lotteryTime)
          }
        })
      },
      // 处理缓存
      handleCache () {
        ...
        localStorage.removeItem('onlyEdit')
      ...
      modifyParam () {
        ...
        // 虚拟参与人
          if(this.param.source === 0){//平台选品下考虑虚拟参与人
            if(this.param.rebotNum === ''){//平台选品下，虚拟参与人限制在0-99999999
              this.$message.warning("请输入0-99999999人的虚拟参与人值");return
            }
      ...
      goNext () {
        if(this.modifyParam()){
          ...
          this.$router.push('/luckDraw/luckDrawList/edit_two/'+this.lotteryId)
        }
      },
      goBack () {
