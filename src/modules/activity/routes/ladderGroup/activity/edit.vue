<template>
  <div class="cms">
      ...
      <el-form label-width="150px" size="small">
        ...
        <template v-if="isEditing">
          <el-form-item label="新客补贴：">
             <el-input class="w200" v-model="detail.newerSubsidy" v-only-num="detail.newerSubsidy" :max-len="2"></el-input> 元/人
          </el-form-item>
          <el-form-item label="拼团人数：">
             <el-input class="w200" v-model="detail.groupSize" v-only-num="detail.groupSize" :max-len="2"></el-input>
          </el-form-item>
          <el-form-item label=" ">
             <el-button type="primary" @click="updateActivity">保存</el-button>
             <el-button @click="handleCancel">取消</el-button>
             <div class="font-danger">编辑阶梯团信息需要重新审核</div>
          </el-form-item>
        </template>
        <!-- 正在生效的阶梯团信息 -->
        <template v-else>
          ...
          <el-form-item label="生效拼团人数：">
             {{ detail.groupSize | placeholder }} (审核值：{{ detail.verifyingGroupSize }})
          </el-form-item>
          <el-form-item label=" " v-if="detail.auditStatus === 3 || detail.auditStatus === 4">
             <el-button type="primary" @click="isEditing = !isEditing">编辑</el-button>
             <div class="font-danger">编辑阶梯团信息需要重新审核</div>
          </el-form-item>
        </template>
      </el-form>
    ...
    <el-card class="box-card ma-t-10">
      ...
      <el-form label-width="150px" size="small">
        <el-form-item label="活动名称：">
           <el-input class="w200" v-model.trim="detail.activityName" :maxLength="20"></el-input>
        </el-form-item>
    ...
    <el-card class="box-card ma-t-10">
      <el-form :inline="true" label-width="150px" size="small">
        ...
        <el-form-item label="上下架状态：">
          <el-select v-model="searchParam.shelveStatus" class="w200">
        ...
        <el-form-item label="增值服务：">
          <el-select v-model="searchParam.addValServer" class="w200">
            <el-option label="全部" value=""></el-option>
        ...
        <el-form-item label="商品审核状态：">
          <el-select v-model="searchParam.productAuditStatus" class="w200">
            <el-option label="全部" value=""></el-option>
        ...
        <el-form-item label="是否新品：">
          <el-select v-model="searchParam.isNew" class="w200">
            <el-option label="全部" value=""></el-option>
    ...
    <select-product class="ma-b-40" :selectable-list="selectablelist" :selected-list="selectedlist" :selectable-cols="selectableCols" :selected-cols="selectedCols" @change="selectedlist=$event" ref="selectPro" :total="1000">
      <!-- 分页信息 -->
      ...
      <!-- 填充到可选择列表中去 -->
      <div slot="selectable-shelveStatusName" slot-scope="scope">
        {{ scope.col.shelveStatusName }}
      </div>
      <div slot="selectable-addValServerName" slot-scope="scope">
        {{ scope.col.addValServerName }}
      </div>
      <div slot="selectable-pAuditStatusName" slot-scope="scope">
        {{ scope.col.pAuditStatusName }}
      </div>
      <div slot="selectable-isNewName" slot-scope="scope">
        {{ scope.col.isNewName }}
      </div>
      <!-- 填充到已选择列表中去 -->
      <div slot="selected-shelveStatusName" slot-scope="scope">
        {{ scope.col.shelveStatusName }}
      </div>
      <div slot="selected-addValServerName" slot-scope="scope">
        {{ scope.col.addValServerName }}
      </div>
      <div slot="selected-pAuditStatusName" slot-scope="scope">
        {{ scope.col.pAuditStatusName }}
      </div>
      <div slot="selected-isNewName" slot-scope="scope">
        {{ scope.col.isNewName }}
      </div>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        selectableCols:[
          {label:'上下架状态',prop:'shelveStatusName'},
          {label:'增值服务',prop:'addValServerName'},
          {label:'商品审核状态',prop:'pAuditStatusName'},
          {label:'是否新品',prop:'isNewName'}
        ],
        selectedCols:[
          {label:'上下架状态',prop:'shelveStatusName'},
          {label:'增值服务',prop:'addValServerName'},
    ...
    watch: {
      detail (nv,ov) {
        this.dateObj = [nv.beginTime, nv.endTime]
      }
    },
    components:{
      SuctionBottom,
    ...
    methods: {
      ...
      handleChange (rel) {// 处理时间范围变化
        this.detail.beginTime = rel ? rel[0] : ''
        this.detail.endTime = rel ? rel[1] : ''
      },
      clearParam () {// 初始化查询信息
      ...
      checkParam () {// 校验参数有效性
        ...
        if(new Date(this.detail.endTime+":59").getTime() <= Date.now()){
      ...
      saveTask () { // 保存信息
        if(this.checkParam()){
          this.$service.activity.ladderGroup.updateActivity({
            activityName:this.detail.activityName,
            beginTime:this.detail.beginTime,
            endTime:this.detail.endTime,
            activityId:this.detail.activityId,
            productList: this.selectedlist.map(item => {
              return {
      ...
      updateActivity () {// 更新阶梯团基础信息
        if(this.detail.newerSubsidy < 2){
          this.$message.warning('请输入2-99新客补贴');return false
        }
        if(this.detail.groupSize < 2 || this.detail.groupSize > 20){
          this.$message.warning('请输入2-20拼团人数');return false
        }
        this.$service.activity.ladderGroup.updateVerifyTerm({
          newerSubsidy:this.detail.newerSubsidy,
          groupSize:this.detail.groupSize,
          activityId:this.detail.activityId
        }).then(res => {
