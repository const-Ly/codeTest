  ...
  <div class="cms">
    <el-card class="box-card">
       <el-form :inline="true" label-width="150px" size="small">
        ...
        <el-form-item label="参加特权金：">
          <el-select v-model="param.joinNovicePrivilegedGold" class="w300">
    ...
    <el-card class="box-card ma-t-10">
      ...
      <el-table :data="proList" style="width: 100%" size="small">
        ...
        <el-table-column label="商品扣点">
          <template slot-scope="scope">
            <template v-if="scope.row.productCost !== null && scope.row.productCost !== ''">
    ...
    <el-card class="box-card ma-t-10 pa-b-30">
      ...
      <el-table :data="checkedProList" style="width: 100%" size="small">
        ...
        <el-table-column label="*新用户限额">
          <template slot-scope="scope">
            <el-input v-model="scope.row.userCommonPrivilegeDiscountLimit" v-only-num:userCommonPrivilegeDiscountLimit="checkedProList" :key="scope.$index" :max-len="2" class="w80"></el-input> &nbsp;元
          </template>
        </el-table-column>
        <el-table-column label="*老客限额">
          <template slot-scope="scope">
            <el-input v-model="scope.row.customerCommonPrivilegeDiscountLimit" v-only-num:customerCommonPrivilegeDiscountLimit="checkedProList" :key="scope.$index" :max-len="2" class="w80"></el-input> &nbsp;元
          </template>
        </el-table-column>
    ...
    methods:{
      ...
      // 获取可选择商品
        getSelectableProductList (pageNow) {
          ...
          this.$service.activity.together.getPreActivityProductByPage(this.param).then(res => {
        ...
        clearParam () {
          ...
          this.param.changeCategoryType = 2
          this.param.joinNovicePrivilegedGold = ""
      ...
      // 操作
        ...
        //保存商品
        saveTask () {
          let _filter = false, _filterP = false, _filterB = false;
          if(this.checkedProList.length === 0){
            ...
            let _param = this.checkedProList.map(item => {
              return {
                ...
                userCommonPrivilegeDiscountLimit: item.userCommonPrivilegeDiscountLimit,
                customerCommonPrivilegeDiscountLimit: item.customerCommonPrivilegeDiscountLimit,
                productForm: item.productForm
              }
            })
            _param.forEach(item => {
              if(item.platformDiscount > 99 || item.platformDiscount === "") {
                _filter = true;
              } else if(item.platformDiscount >= 13 && (item.userCommonPrivilegeDiscountLimit > 1 || item.customerCommonPrivilegeDiscountLimit > 1)) {
                _filterB = true;
              }
              if(item.userCommonPrivilegeDiscountLimit === null ||  item.userCommonPrivilegeDiscountLimit > 50 || item.userCommonPrivilegeDiscountLimit === "") {
                _filterP = true;
              }
              if(item.customerCommonPrivilegeDiscountLimit === null || item.customerCommonPrivilegeDiscountLimit > 50 || item.customerCommonPrivilegeDiscountLimit === ""){
                _filterP = true;
              }
            })
            if(_filter){
            ...
            // }
            this.$service.activity.together.batchSaveActivityProduct(_param).then(res => {
              this.$router.back()
            })
          }
        }
