<template>
  <div class="cms">
    <el-card class="box-card">
      ...
      <el-form label-width="180px" size="small">
          <el-form-item label="活动状态：">
            <el-radio-group v-model="togGroupActivitySwitch">
              <el-radio :label="1" @click.native.prevent="handleSaveActivity(1,1)">开启</el-radio>
              <el-radio :label="0" @click.native.prevent="handleSaveActivity(0,0)">关闭</el-radio>
            </el-radio-group>
          </el-form-item>
        <el-form-item label="参团对象：">
          <el-radio-group v-model="togGroupActivityUserType">
            <el-radio :label="1" @click.native.prevent="handleSaveActivity(2,1)">新用户</el-radio>
            <!-- <el-radio :label="2" @click.native.prevent="handleSaveActivity(3,2)">老客</el-radio> -->
            <el-radio :label="3" @click.native.prevent="handleSaveActivity(4,3)">全部</el-radio>
        </el-radio-group>
        </el-form-item>
      </el-form>
      ...
      <el-form :inline="true" label-width="150px" size="small">
        ...
        <el-form-item label="商品扣点：">
          <div class="w200">
            <el-input class="w80" v-only-num="param.productCostStart" v-model="param.productCostStart"></el-input>
            <span class="range-input-split">至</span>
            <el-input class="w80" v-only-num="param.productCostEnd" v-model="param.productCostEnd"></el-input>
          </div>
        </el-form-item><br>
        ...
        <el-form-item label="负毛利商品：">
          <el-select v-model="param.negativeProfitStatus" class="w200">
            <el-option label="全部" value=""></el-option>
        ...
        <el-form-item label="代理商：">
          <factorLenovoSearch v-model="param.channelCode" class="w200"></factorLenovoSearch>
        </el-form-item><br>
        <el-form-item label=" ">
    ...
    <el-card class="box-card ma-t-10">
      <div slot="header" class="clearfix">
        <span class="line-32">一起拼商品</span>
        <el-button class="ma-l-20" type="primary" size="small" @click="batchAudit">批量审核</el-button>
        <file-upload @success="getToggroupProductList()" action="/activity/toggroupChannelPage/import">
          <el-button type="danger" size="small" slot="handle">导入</el-button>
        </file-upload>
        <el-button class="ma-l-10" type="success" size="small" @click="handleExport">导出</el-button>
      ...
      <el-table :data="proList" style="width: 100%" @selection-change="handleSelectionChange" size="small">
        ...
        <el-table-column label="新用户/老客限额" width="130">
          <template slot-scope="scope">
            <span>
    ...
    <!-- 全局配置 -->
    <el-dialog :visible.sync="showConfig" width="30%">
      <span>{{configTip}}</span>
      <span slot="footer" class="dialog-footer">
        <el-button size="small" type="primary" @click="handleOkConfig">确 定</el-button>
        <el-button size="small" @click="showConfig = false">取 消</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog :visible.sync="showAuditDetail" width="60%" :title="title" class="mapBox">
      <el-form :inline="true" label-width="100px" size="small">
        ...
        <template v-if="mapStatus === 2">
          <el-form-item label="新用户限额：">
            <el-input v-model="auditDetail.userCommonPrivilegeDiscountLimit" v-only-num="auditDetail.userCommonPrivilegeDiscountLimit" :max-len="2" class="input-mini"></el-input> &nbsp;元
          </el-form-item>
          <el-form-item label="老客限额：">
            <el-input v-model="auditDetail.customerCommonPrivilegeDiscountLimit" v-only-num="auditDetail.customerCommonPrivilegeDiscountLimit" :max-len="2" class="input-mini"></el-input> &nbsp;元
          </el-form-item>
        </template>
        <template v-else>
  ...
  export default {
    ...
    data () {
      return {
        ...
        showConfig:false,
        _target:null,
        configTip:"",//全局配置二次确认提示
        togGroupActivitySwitch:0, // 活动状态
        togGroupActivityUserType:1, // 参团对象
        ...
        param:{
          ...
          pageSize:20,
          pageIndex:1,
          sellStatus:"",	// 可售状态(1可售2不可售 3--)
          negativeProfitStatus:"",
          isNew: '',
          productForm: 1,
          channelCode: ''
        },
        ...
        auditDetail:{
          ...
          userCommonPrivilegeDiscountLimit: "", // 新用户通用特权金使用限额
          customerCommonPrivilegeDiscountLimit: "", // 老客通用特权金使用限额
    ...
    methods:{
      //全局配置
        // 切换选项
        handleSaveActivity (val,old) {
          let self = this
          self._target = val
          switch (val) {
            case 0:
              self.configTip = "确认关闭一起拼活动吗?";break;
            case 1:
              self.configTip = "确认开启一起拼活动吗?";break;
            case 2:
              self.configTip = "你确认修改参团对象为新客吗？";break;
            case 3:
              self.configTip = "你确认修改参团对象为老客吗？";break;
            default:
              self.configTip = "你确认修改参团对象为全部吗？";
          }
          if((val <2 && self.togGroupActivitySwitch !== old) || (val >=2 && self.togGroupActivityUserType !== old) ){
        ...
        handleOkConfig () {
          ...
          let param = {}
          if(self._target<2){
            param.togGroupActivitySwitch = self._target
            param.togGroupActivityUserType = null
          }else{
            param.togGroupActivitySwitch = null
            param.togGroupActivityUserType = self._target - 1
          }
          this.$service.activity.together.saveActivity(param).then(res => {
        ...
        // 获取全局配置
        getActivityDetail () {
          this.$service.activity.together.getActivityDetail().then(res => {
            this.togGroupActivitySwitch = res.togGroupActivitySwitch
            this.togGroupActivityUserType = res.togGroupActivityUserType
          })
        },
      ...
      // 获取所有类目信息
        getBackendCategoryTree () {
          this.$service.common.getBackendCategoryTree().then(res => {
            this.categoryList = res || [] // todo 记得去掉
          })
        },
        //选择的三级联动信息
        handleChange (val) {
      ...
      // 获取一起拼商品
        getToggroupProductList (pageNow) {
          ...
          this.$service.activity.together.getActivityProductByPage(self.param).then(res => {
        ...
        clearParam () {
          ...
          this.param.platformDiscountStart = ""
          this.param.platformDiscountEnd = ""
          this.param.productCostStart = ""
          this.param.productCostEnd = ""
          this.param.shortTitle = ""
          this.param.productId = ""
          this.param.supplierDiscountStatus = ""
          this.param.auditStatus = ""
          this.param.sellStatus = ""
          this.param.negativeProfitStatus = ""
          this.param.isNew = ""
      ...
      batchAudit () {
        if(this.checkedList.length === 0){
          ...
          this.$confirm('你确定审核通过所选商品吗？', '提示', {
            ...
            this.$service.activity.together.batchFirstAuditPassByApplyNo(this.checkedList).then(res => {
              if(res.length>0){
      ...
      handleRemove (row) {
        this.$confirm('你确认删除该优惠吗？', '提示', {
          ...
          this.$service.activity.together.deleteByApplyNo({productId:row.productId,applyNo:row.applyNo,productForm:row.productForm}).then(res => {
            this.getToggroupProductList()
          })
        })
      },
      ...
      handleMap (row,type) {
        ...
        this.$service.activity.together.getApplicationFormsByApplyNo({applyNo:row.applyNo,productId:row.productId}).then(res => {
      ...
      handleNewPlatformDiscount () {
        if(this.newPlatformDiscount>99 || this.newPlatformDiscount === ""){
          this.$message.warning("请输入0-99新平台优惠");return;
        }
        if(this.auditDetail.userCommonPrivilegeDiscountLimit == null || this.auditDetail.userCommonPrivilegeDiscountLimit > 50 || this.auditDetail.userCommonPrivilegeDiscountLimit === ''){
          this.$message.warning("请输入0-50新/老客限额");return;
        }
        if(this.auditDetail.customerCommonPrivilegeDiscountLimit == null ||this.auditDetail.customerCommonPrivilegeDiscountLimit > 50 || this.auditDetail.customerCommonPrivilegeDiscountLimit ===''){
          this.$message.warning("请输入0-50新/老客限额");return;
        }
        if(this.newPlatformDiscount>13 && (this.auditDetail.userCommonPrivilegeDiscountLimit > 1 || this.auditDetail.customerCommonPrivilegeDiscountLimit > 1)){
          this.$message.warning("平台优惠大于等于13%时，新用户限额/老客限额必须小于等于1");return;
        }
          let _param = {
            productId: this.auditDetail.productId,
            userCommonPrivilegeDiscountLimit: this.auditDetail.userCommonPrivilegeDiscountLimit,
            customerCommonPrivilegeDiscountLimit: this.auditDetail.customerCommonPrivilegeDiscountLimit,
        ...
        if(this.auditDetail.productForm === 1){
          ...
          this.$service.activity.together.judgeNegativeProfit(_param).then(res => {
      ...
      handleExport () {
        ...
        this.$service.activity.together.togGroupActivityExport(_param).then(res => {
          window.open(res)
        })
      },
      // 编辑确认保存接口的api管理
      apiUpdatePlatformDiscountByApplyNo(){
        this.$service.activity.together.updatePlatformDiscountByApplyNo({
          productId:this.auditDetail.productId, 
          applyNo:this.auditDetail.applyNo,
          oriPlatformDiscount:this.auditDetail.oriPlatformDiscount, 
          supplierDiscount:this.auditDetail.supplierDiscount, 
          platformDiscount:this.newPlatformDiscount,
          userCommonPrivilegeDiscountLimit: this.auditDetail.userCommonPrivilegeDiscountLimit,
          customerCommonPrivilegeDiscountLimit: this.auditDetail.customerCommonPrivilegeDiscountLimit,
          productForm: this.auditDetail.productForm
        }).then(res => {
      ...
      apiFirstAuditByApplyNo () {
        this.$service.activity.together.firstAuditByApplyNo({
          productId:this.auditDetail.productId, 
          applyNo:this.auditDetail.applyNo, 
          firstAuditStatus:this.audisResult, 
          productForm: this.auditDetail.productForm
        }).then(res => {
          this.showAuditDetail = false
          this.showNegativeProfit = false
      ...
      submitBatchAuditFail(){
        this.$service.activity.together.batchFirstAuditRejectByApplyNo(this.batchAuditFailList).then(res => {
