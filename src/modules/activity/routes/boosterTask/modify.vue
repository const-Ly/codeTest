  ...
  <el-card class="box-card">
		...
		<el-form label-width="150px">
			<el-form-item label="助力ID：">
				<span v-text="taskId"></span>
			</el-form-item>
			<el-form-item label="任务名称：">
				<el-col :span="6">
					<el-input v-model.trim="taskName" :maxlength="40" @blur="testTaskName" placeholder="请输入1-40字符任务名称" clearable></el-input>
				</el-col>
			</el-form-item>
...
export default {
  ...
  data () {
    return {
			activeName: [],  // 手风琴是否默认展开
			initExpand: [], // 发起助力用户内等级是否展开
			joinExpand: [], // 可参与助力用户内等级是否展开
			failExpand: [], // 失败助力用户内等级是否展开
			getRowKeys(row){ // 每一列的唯一标识
				return row.userType;
			},
      showCopyRightDialog: false, // 是否复制权益
			luanIndeter: false, // 失败权益是否为半选状态
			failIndeter: false, // 新用户权益是否为半选状态
			handIndeter: false, // 可参与用户是否半选
			failCheck: true, // 失败权益默认全选
			luanCheck: false, // 新用户权益默认为不全选
			handCheck: false, // 可参与助力默认不全选
  ...
  created () {
  	this.$service.activity.boosterTask.taskDetail({taskId:this.$route.params.id}).then(res => {
			...
			this.validPeriod = res.validPeriod;

			this.initiateUserRightsList = res.initiateUserRightsList;
			this.joinUserRightsList = res.joinUserRightsList;
			this.failureRightsList = res.failureRightsList;

			this.luanchList = this.initiateUserRightsList.map(item => item.userType.toString())
			this.handInList = this.joinUserRightsList.map(item => item.userType.toString())
			this.failInList = this.failureRightsList.map(item => item.userType.toString())

			this.luanChange(this.luanchList);
			this.handChange(this.handInList);
			this.failChange(this.failInList);
  	})
  },
  components:{
  	SuctionBottom
