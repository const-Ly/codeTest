  ...
  <div class="cms">
      ...
      <el-form ref="form" label-width="150px" size="small" class="input-normal">
        ...
        <el-form-item label="">
          ...
          <el-button type="primary" @click="editTemplate">提交保存</el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <!-- 预览 -->
  ...
  export default {
    ...
    methods:{
      ...
      handlePreviewOriginal (val) {
	  	...
	  	getTemplate(){
        let self = this
	  		self.$service.activity.faceScore.queryTemplateById({templateId:self.$route.params.id}).then(res => {
          this.param = res
          this.param.templateId = self.$route.params.id	//主题id
       	})
	  	},
	  	// 编辑模板
	  	editTemplate() {
        ...
        self.$service.activity.faceScore.updateTemplate(self.param).then(res => {
          this.$router.back()
       	})
      },
      // 获取主题列表
