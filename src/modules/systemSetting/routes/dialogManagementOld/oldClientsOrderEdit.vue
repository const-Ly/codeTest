    ...
    <template v-for="(item, index) in detail.configDetailList">
      <div :key="item.seqNum" class="card">
        <div class="card-title">
          <div class="txt">{{item.seqNumName}}（<span v-if="item.effectStatus">生效</span><span v-else>不生效</span>）</div>
          <div class="btn-group">
            <div class="expend">
        ...
        <div v-if="item._isExpand" class="card-content">
          <el-form ref="baseForm" :model="detail" label-width="160px">
            <el-form-item class="text-label" label="是否生效：">
              <el-radio-group v-model="item.effectStatus">
                <el-radio :label="true">生效</el-radio>
                <el-radio :label="false">不生效</el-radio>
              </el-radio-group>
              <p class="tips">不生效该节点总体不弹出弹窗</p>
            </el-form-item>
            <div v-show="item.effectStatus">
              <el-form-item label="显示位置：">
                <el-checkbox-group v-model="item.popLocation">
                  <el-checkbox :label="1">首页</el-checkbox>
                  <el-checkbox :label="2">个人中心</el-checkbox>
                  <el-checkbox :label="3" v-if="item.seqNum === 1 || item.seqNum === 3 || item.seqNum === 5 || item.seqNum === 7">退款详情页（仅APP）</el-checkbox>
                  <el-checkbox :label="4" v-if="item.seqNum === 6">订单列表页</el-checkbox>
                  <el-checkbox :label="5" v-if="item.seqNum === 6">订单详情页</el-checkbox>
                </el-checkbox-group>
              </el-form-item>
              ...
              <el-form-item class="text-label" label="弹窗图片：">
                ...
                <Upload
                  ...
                  :type="['.jpg', '.png', '.gif']"
                  :size='300'
                  :image-url="item.secondPopImgUrl"
                  @success="item.secondPopImgUrl = $event">
                </Upload>
                </div>
                <div class="tips mar-top">图一为首版（APP5.5.0）弹层图片，图二为首版之后（APP版本大于5.5.0）弹层图片</div>
                <div class="tips">支持jpg、png、gif格式文件，小于300k</div>
              </el-form-item>
  ...
  methods: {
    ...
    submitModule () {
      ...
      params.configDetailList = detail.configDetailList.map(item => {
        ...
        subItem.seqNumName = item.seqNumName
        subItem.effectStatus = item.effectStatus
        subItem.popLocation = item.popLocation
        subItem.closeButtonEffectStatus = item.closeButtonEffectStatus
        subItem.popImgUrl = item.popImgUrl
        subItem.secondPopImgUrl = item.secondPopImgUrl
      ...
      for (let i = 0; i < arr.length; i ++) {
        const current = arr[i]
        if(!current.effectStatus) continue
        if (!current.popImgUrl || !current.secondPopImgUrl) {
        ...
        if (current.countDownStatus) {
          if (current.countDownHour === null ||
            ...
            this.$message.warning(`请输入${current.seqNumName}有效的显示倒计时时间，小时0-99，分钟0-59，小时分钟不可同为0`)
            return false
          }
        }
        for (let k = 0; k < current.routeList.length; k ++) {
          ...
          if (currentRoute.checkedStatus && (currentRoute.jumpType === '' || currentRoute.jumpType === null)) {
            this.$message.warning(`请选择填写${current.seqNumName}${platformType[currentRoute.clientType]}跳转类型`)
            return false
          }
          if (currentRoute.checkedStatus && currentRoute.jumpType === 1 && (currentRoute.jumpPath === '' || currentRoute.jumpPath === null)) {
            this.$message.warning(`请输入${current.seqNumName}${platformType[currentRoute.clientType]}页面路由`)
            return false
          }
        }
      }
