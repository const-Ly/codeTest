  ...
  <el-card class="box-card ma-t-10">
    <el-form label-width="160px">
      ...
      <el-form-item label="是否循环：">
        <el-radio-group v-model="param.cycleStatus" :disabled="editState">
          <el-radio :label="1">循环</el-radio>
          <el-radio :label="0">不循环</el-radio>
        </el-radio-group>
        <p v-if="!editState" class="font-info">不循环为单次弹出，仅出现一次</p>
        <p v-if="!editState" class="font-info">循环为重复弹出，多次的逐条循环，单条的每次触发均弹出</p>
      </el-form-item>
    ...
    methods: {
      ...
      getDetail () {
        ...
        this.$service.systemSetting.dialogManagement[_url](_param).then( res => {
          this.param = res.popCodeNode3 || {}
        })
      },
      save () {
        this.$service.systemSetting.dialogManagement.visitorUpdateParentConfig(this.param).then( res => {
