<template>
  <el-card class="card">
    ...
    <div>
      ...
      <div class="orderWrap" v-for="(item, idx) in configNode.payedOrderPopConfigList" :key="idx">
        <div class="clearfix orderHeader">
          <span class="ma-r-20">{{item.orderDiscountTypeName}}</span>
            ...
            <div v-for="(o, num) in item.routeList" :key="num" class="ma-b-10">
              ...
              <div v-show="o.checkedStatus" class="inline-block x-box">
                ...
                <el-select class="ma-l-10 w200" v-model="o.jumpType" placeholder="请选择" :disabled="editState">
                  <el-option v-show="o.clientType === 3" label="页面链接" :value="4"></el-option>
                  <el-option v-show="o.clientType !== 3" label="页面路由" :value="1"></el-option>
                  <el-option label="首页" :value="2"></el-option>
                  <el-option label="关闭弹窗" :value="3"></el-option>
                </el-select>
                <span class="ma-l-10" v-if="editState && [1, 4].indexOf(o.jumpType) > -1">{{o.jumpPath}}</span>
                <el-input class="w300 ma-l-10" v-show="!editState && [1, 4].indexOf(o.jumpType) > -1" v-model.trim="o.jumpPath" maxlength="999"></el-input>
              </div>
            </div>
          </el-form-item>
        </el-form>
  ...
  data () {
    return {
      ...
      configNode: {
        ...
        payedOrderPopConfigList: [
          {
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 1, // 订单类型枚举当前仅有：0、41、42、5
            orderDiscountTypeName: '普通订单未使用特权金', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            ...
            routeList: [
              {
                ...
                clientType: 3, // 客户端类型，1-ios；2-Android； 4-小程序
                clientTypeName: 'M站', // 客户端类型名称
                checkedStatus: false, // 客户端是否被选中，false-未选中，true-选中
                jumpType: '', // 跳转类型，1-页面路由；2-首页；3-关闭弹窗
                jumpPath: '' // 跳转路径
              }, {
            ...
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 2, // 订单类型枚举当前仅有：0、41、42、5
            orderDiscountTypeName: '普通订单使用通用特权金', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
            ...
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 3, // 订单类型枚举当前仅有：0、41、42、5
            orderDiscountTypeName: '一起拼参团', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
            ...
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 4, // 订单类型枚举当前仅有：0、41、42、5
            orderDiscountTypeName: '一起拼开团', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
            ...
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 5, // 订单类型枚举当前仅有：0、41、42、5
            orderDiscountTypeName: '新手专享', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
            ...
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 6, // 订单类型枚举当前仅有：0、41、42、5
            orderDiscountTypeName: '新手特权金', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
          ...
          },
          {
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 7, 
            orderDiscountTypeName: '普通订单使用立减金', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
          ...
          },
          {
            orderSwitch: false, // 默认关闭状态
            orderDiscountType: 8, 
            orderDiscountTypeName: 'x元购订单', // 订单类型名称
            popImgUrl: '', // 弹窗图片url地址
            hangUpAwardType: 0, // 挂接类型枚举，当前仅有：0、1、2、3
            countDownStatus: false, // 是否显示倒计时
            countDownHour: '', // 倒计时-小时（倒计时开启时，有效）
  ...
  methods: {
    ...
    getPageInfo () {
      ...
      this.$service.systemSetting.dialogManagement[_url](param).then(res => {
        if (res) {
          this.configNode = res.sonPopCodeNode5
    ...
    checkout () {
      // 成单次数
      if (this.configNode.minNum === '' || this.configNode.minNum === null ||
        this.configNode.maxNum === '' || this.configNode.maxNum === null) {
        this.$message.warning('请输入支付次数')
        return false
      }
      this.configNode.minNum = parseInt(this.configNode.minNum)
      this.configNode.maxNum = parseInt(this.configNode.maxNum)
      if (this.configNode.minNum > this.configNode.maxNum) {
        this.$message.warning('支付次数最小值必须小于等于最大值')
        return false
      }
      if (!this.datePicker) {
      ...
      if (typeof this.datePicker[0] !== 'string') {
        this.configNode.effectStartTime = this.$utils.date2str(this.datePicker[0])
        this.configNode.effectEndTime = this.$utils.date2str(this.datePicker[1])
      } else {
        this.configNode.effectStartTime = this.datePicker[0]
        this.configNode.effectEndTime = this.datePicker[1]
      }
      // 订单循环
      let orderPop = null
      for (let o = 0; o < this.configNode.payedOrderPopConfigList.length; o++) {
        orderPop = this.configNode.payedOrderPopConfigList[o]
        // 订单开启
        if (orderPop.orderSwitch) {
          ...
          if (orderPop.countDownStatus) {
            if (orderPop.countDownHour==='' || orderPop.countDownHour===null ||
              ...
              this.$message.warning('请输入'+ orderPop.orderDiscountTypeName +'有效的显示倒计时时间，小时0-99，分钟0-59，小时分钟不可同为0')
              return false
            }
          ...
          for (let r=0; r<orderPop.routeList.length; r++) {
            ...
            if (route.checkedStatus) {
              // 未选择跳转类型
              if (!route.jumpType) {
                this.$message.warning('请选择填写'+ orderPop.orderDiscountTypeName + type +'跳转类型')
                return false
              }
              // 跳转类型为页面路由时，路由为空
              if ([1, 4].indexOf(route.jumpType) > -1 && !route.jumpPath) {
                this.$message.warning('请输入页面' + (route.clientType === 3 ? '链接' : '路由'))
                return false
              }
            }
          }
    ...
    submit () {
      ...
      if (!param) return
      this.$service.systemSetting.dialogManagement.payedAddOrUpdateSonConfig(param)
      .then(res => {
        if (res) this.goback()
      })
    },
    goback () {
      this.$router.push('/dialogManagement/paid/childManagement/' + this.configNode.sonPopCode)
    }
  },
}
</script>
