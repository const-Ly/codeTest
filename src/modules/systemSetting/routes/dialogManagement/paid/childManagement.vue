<template>
  <el-card class="box-card ma-t-10">
    <div slot="header" class="clearfix ma-b-20">
      <span>弹窗页面： {{tit}}</span>
      <router-link class="fr" :to="'/dialogManagement/paid/childAdd/'+ param.sonPopCode">
        <el-button size="small" type="primary">添加</el-button>
      </router-link>
    </div>
    <el-tabs v-model="effectStatus" @tab-click="handleClick">
      <el-tab-pane label="生效中" name="2"></el-tab-pane>
      <el-tab-pane label="待开始" name="1"></el-tab-pane>
      <el-tab-pane label="已失效" name="3"></el-tab-pane>
    </el-tabs>
    
    <el-table class="ma-b-20" :data="list" style="width:100%" size="small">
      <el-table-column label="编号" prop="sonNodeId"></el-table-column>
      <el-table-column label="支付次数">
        <template slot-scope="scope">
          {{scope.row.minNum}} - {{scope.row.maxNum}}
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <router-link :to="'/dialogManagement/paid/childEdit/'+ param.sonPopCode +'/' + scope.row.sonNodeId">
